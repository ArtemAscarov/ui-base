(function () {
  ...
  var ru = {
    ...
    "masthead.lede":
      "Шесть компонентов, написанных вручную и собранных под тот стек, которым вы правда пользуетесь. Одна команда копирует исходники в ваш проект — ничего не тянется в зависимости, и дальше файлы ваши.",
    "tally.components": "Компонентов",
    "tally.variants": "Вариантов",
    "tally.files": "Файлов",

    "install.eyebrow": "Одна команда — и файлы ваши",
    "install.lede":
      "CLI копирует исходники в ваш проект и на этом заканчивается. В зависимости ничего не добавляется, кроме React — и только если вы попросили React- или Next-вариант.",
    "install.flags":
      "Флаги: --vanilla, --react, --next выбирают стек; --ts и --tw — вариант; --dry-run показывает, что будет записано, ничего не записывая.",

    "spec.get": "Забрать",
    "get.note":
      "Разметка — это фрагмент: вставьте его в свою страницу и подключите стили и скрипт.",
    "picker.stack": "Стек",
    "picker.flavor": "Вариант",
    "picker.files": "Файлы варианта",
    "copy.label": "Копировать",
    "copy.aria": "Скопировать в буфер обмена",
  ...
  if (dictionary) {
    ...
    apply("data-i18n-copied", function (element, value) {
      element.dataset.copiedLabel = value;
    });
  }
