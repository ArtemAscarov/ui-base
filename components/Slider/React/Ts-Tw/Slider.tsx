...
export function Slider({
  ...
  const [internal, setInternal] = useState<SliderValue>(defaultValue);
  ...
  function percent(raw: number): number {
  ...
  function apply(index: number, raw: number) {
    const next = [...values];
    next[index] = snap(raw);
  ...
  function commit(next: number[], type: "input" | "change"): void {
    const result: SliderValue = isRange ? [next[0], next[1]] : next[0];
  ...
  function nearest(raw: number): number {
    if (!isRange) return 0;
  ...
  function handlePointerDown(event: PointerEvent<HTMLDivElement>): void {
    ...
    const thumb = (event.target as HTMLElement).closest<HTMLDivElement>(
      "[data-slider-thumb]"
    );
